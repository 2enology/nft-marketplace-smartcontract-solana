...
programCommand('plist')
  ...
  .action(async (directory, cmd) => {
    ...
    await listPNftForSale(new PublicKey(address), parseFloat(price_sol) * LAMPORTS_PER_SOL);
  });
...
programCommand('create_auction')
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, address, start_price, min_increase, duration, reserve } = cmd.opts();
...
programCommand('update_reserve')
  ...
  .action(async (directory, cmd) => {
    ...
    await updateReserve(new PublicKey(address), parseFloat(start_price) * LAMPORTS_PER_SOL);
  });
