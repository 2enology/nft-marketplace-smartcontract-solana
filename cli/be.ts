...
const main = async () => {
  await getAllTransactions(
    new PublicKey("C29hER4SXQr3atHsuCrRmLAkXBpxvfLMCNeXg2TRTd9o")
  );
...
export const getAllTransactions = async (contractId: PublicKey) => {
    ...
    case 10:
      ...
      for (let i = 0; i < innerIx.length; i++) {
        ...
        if (
          ...
          ins.parsed.info.source === accountKeys[0].toBase58()
        ) {
    ...
    case 16: {
      ...
      for (let i = 0; i < innerIx.length; i++) {
        ...
        if (
          ins.program === "system" &&
          ins.parsed.type === "transfer" &&
          ins.parsed.info.source === accountKeys[10].toBase58()
        ) {
          price += ins.parsed.info.lamports;
        }
      }
    ...
    case 20: {
      ...
      for (let i = 0; i < innerIx.length; i++) {
        ...
        if (
          ins.program === "system" &&
          ins.parsed.type === "transfer" &&
          ins.parsed.info.source === accountKeys[6].toBase58()
        ) {
          price += ins.parsed.info.lamports;
        }
      }
