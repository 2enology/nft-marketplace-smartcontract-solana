...
describe('Contract Creation', async () => {
  ...
  it('Admin can change the Marketplace Fee', async () => {
    ...
    assert(globalInfo.marketFeeToken.toNumber() == 12, "Token Fee is not 0.12%");
  });
  it('Admin can add himself as team', async () => {
    ...
    assert(globalInfo.treasuryRate[0].toNumber() == 500, "Superowner is treasury rate is 5%");
  });
  ...
  it('Admin can add himself as team', async () => {
    ...
    assert(globalInfo.teamCount.toNumber() == 1, "No team treasury added");
    assert(globalInfo.teamTreasury[0].toBase58() == superOwner.publicKey.toBase58(), "Superowner is team");
    assert(globalInfo.treasuryRate[0].toNumber() == 30, "Superowner is treasury rate is 0.3% finally");
  });
});
...
describe('user1 Can Purchase NFT', async () => {
  ...
  it('user1 can purchase NFT from user with ABB Token payment', async () => {
    ...
    let fee_amount = Math.floor(nftInfo.priceToken.toNumber() * globalInfo.marketFeeToken.toNumber() / 10_000);
    const userATA = await getAssociatedTokenAccount(user.publicKey, ABB_TOKEN_MINT);
    console.log(await getTokenAccountBalance(userATA, provider.connection), (nftInfo.priceToken.toNumber() - fee_amount) / ABB_TOKEN_DECIMAL, fee_amount, 'Seller ABB Account balance is not same with TokenPrice');
    ...
    let buyerUserInfo = await getUserPoolState(user1.publicKey, program as unknown as anchor.Program);
    assert(buyerUserInfo.address.toBase58() == user1.publicKey.toBase58(), "UserData Address mismatch with User1 Pubkey");
    assert(buyerUserInfo.tradedTokenVolume.toNumber() == nftInfo.priceToken.toNumber() - fee_amount, "UserData TradeVolume is not priceToken");
    let sellerUserInfo = await getUserPoolState(user.publicKey, program as unknown as anchor.Program);
    assert(sellerUserInfo.address.toBase58() == user.publicKey.toBase58(), "UserData Address mismatch with User Pubkey");
    assert(sellerUserInfo.tradedTokenVolume.toNumber() == nftInfo.priceToken.toNumber() - fee_amount, "UserData TradeVolume is not priceToken");
  }); return;
